...
| 🎨 HSL 调色盘 | 可先选择色系（全部/红/黄/绿/青/蓝/品红），再分别精细调整其色相 / 饱和度 / 明度，Photoshop 式彩色滑块；小数值可直接输入数字 |
...
- **HSL**：色相=标准 RGB 色相旋转；饱和度=围绕灰度轴缩放；明度=正值向白场混合（白场保持）、负值向黑场收缩（黑场保持）。三部叠加后一并合成进颜色矩阵。HSL 面板可选择“全部（主）”做整屏调整，或选择 **红/黄/绿/青/蓝/品红** 六个色系分别微调。

> **关于分色系调整**：当前引擎通过 `MagSetFullscreenColorEffect` 只能对整屏应用**一个线性矩阵**，无法逐像素做“只改红色、不改绿色”的真选择性调整。因此选择具体色系时，调整仍作用于全屏，但以该色系自身的色相旋转方向来体现主次差异，属于物理限制下的**近似模拟**；真正的逐色系（selective color）需要逐像素着色器引擎。使用滑块时建议先切回“全部（主）”做整体粗调，再进入具体色系做微调。
